...
void loop() {
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      ...
      if((distance >= _DIST_MIN) && (distance < _DIST_MIDDLE)){
        bright = (distance - _DIST_MIN)*(255 / (_DIST_MIDDLE - _DIST_MIN));
        analogWrite(PIN_LED, 255-bright);
      }
      else{
        bright = ((_DIST_MAX - distance)*(255 / (_DIST_MAX - _DIST_MIDDLE)));
        analogWrite(PIN_LED, 255-bright);
      }
  }
